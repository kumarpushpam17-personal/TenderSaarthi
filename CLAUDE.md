...
## Product roles and workspace boundary

TenderSaarthi has two explicit workspaces:

- **CRPF/Admin workspace** — procurement officers create tenders, upload tender PDFs, review and approve extracted criteria, monitor bidder submissions, run evaluation, inspect verdicts, approve or override results, and generate audit reports.
- **Bidder workspace** — bidder users view open tenders, open tender details, submit firm details plus supporting documents for a selected tender, and track submission status.

Do not blur these roles in docs, routes, prompts, or UI copy. Admins can see all submissions and verdicts. Bidders can see only public tender details and their own submission status. The admin-side `/tenders/{id}/bidders` upload flow is acceptable as a prototype shortcut, but the target product also needs bidder-facing routes such as `/bidder/tenders`, `/bidder/tenders/{id}`, and `/bidder/tenders/{id}/submit`.
...
- **Submission** — the firm profile and document bundle a single bidder uploads against one tender.
- **Admin workspace** — CRPF/procurement officer workflow for tender setup, criterion approval, evaluation, verdict review, and audit reporting.
- **Bidder workspace** — bidder-facing workflow for viewing open tenders and submitting bidder documents.
...
### Adding or changing bidder submission flow
1. Keep public bidder routes under `/bidder/...`; keep officer routes under `/tenders/...` or `/admin/...`.
2. Ensure bidders can only access their own submissions and public tender metadata.
3. Store every submission against exactly one tender and one bidder.
4. Route uploaded documents through `services/bidder/` before any matching or verdict generation.
5. Update `docs/ARCHITECTURE.md`, `docs/DEMO_SCRIPT.md`, and `README.md` if the route or role model changes.
